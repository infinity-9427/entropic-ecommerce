...
// Function to truncate long messages for reduced text display
const truncateMessage = (content: string, maxLength: number = 120): string => {
...
const VoiceAssistant = () => {
  ...
  return (
    <div className="fixed bottom-4 right-4 md:bottom-6 md:right-6 z-50">
      ...
      {isRecording && isOpen && (
        <div className="absolute inset-0 bg-gray-950/98 backdrop-blur-sm z-[70] flex items-center justify-center rounded-2xl">
          ...
          <div className="flex-1 overflow-y-auto p-3 md:p-4 space-y-3 md:space-y-4 bg-black">
            {messages.length === 0 && (
              <div className="text-center py-6 md:py-8">
                <div className="w-10 h-10 md:w-12 md:h-12 rounded-full flex items-center justify-center mx-auto mb-3 bg-purple-700">
                ...
                >
                  <div
                    className={`w-5 h-5 md:w-6 md:h-6 rounded-full flex items-center justify-center flex-shrink-0 ${
                      message.type === "user" ? "bg-purple-600" : "bg-purple-700"
                    }`}
                  >
                    {message.type === "user" ? (
            ...
            <div className="px-3 md:px-4 py-2 bg-gray-900 border-t border-gray-800">
              ...
              >
                {isSpeaking && (
                  <div className="flex space-x-1">
                    <div
                      className="w-1 h-2 md:h-3 bg-purple-500 rounded animate-pulse"
                      style={{ animationDelay: "0ms" }}
                    ></div>
                    <div
                      className="w-1 h-3 md:h-4 bg-purple-500 rounded animate-pulse"
                      style={{ animationDelay: "100ms" }}
                    ></div>
                    <div
                      className="w-1 h-2 bg-purple-500 rounded animate-pulse"
